  ...
  return (
    <>
      ...
      <motion.div
        ...
        className="absolute right-0 top-14 w-80 sm:w-96 bg-surface border border-gray-700 rounded-xl shadow-2xl z-50 overflow-hidden"
      >
        {/* Header */}
        ...
        <div className="max-h-96 overflow-y-auto">
          {notifications.length === 0 ? (
            <div className="p-8 text-center">
              ...
              <p className="text-xs text-gray-600 mt-1">Le nuove notifiche appariranno qui</p>
            </div>
          ) : (
                  ...
                  <div className="flex items-start gap-3">
                    <div
                      className={`w-2 h-2 rounded-full mt-2 flex-shrink-0 ${
                        notification.read ? 'bg-gray-600' : 'bg-primary'
                      }`}
                    />
        ...
        {/* Footer */}
        {notifications.length > 0 && (
          <div className="p-3 border-t border-gray-700 bg-surface/50">
            <button
              onClick={onClearAll}
              className="w-full flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-sm font-medium text-gray-400 hover:text-white hover:bg-surface-light transition-colors"
            >
