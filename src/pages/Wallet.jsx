...
export function Wallet() {
  ...
  const handleDeleteCard = async (id) => {
    ...
    try {
      ...
      alert('Errore nell\'eliminazione della carta: ' + err.message);
    }
  };
    ...
    return (
      <AppLayout>
        <div className="flex items-center justify-center h-64">
          <p className="text-gray-400">Caricamento carte...</p>
        </div>
      </AppLayout>
    );
  }
  ...
  return (
    <AppLayout>
      <div className="space-y-6">
        <div>
          <p className="text-gray-400">Gestisci le tue carte di pagamento</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          {cards.length < 5 && (
            <motion.button
              ...
              className="w-full max-w-[320px] mx-auto h-[192px] rounded-2xl border-2 border-dashed border-gray-700 hover:border-primary transition-all bg-surface/30 hover:bg-surface/50 flex flex-col items-center justify-center gap-3 group"
            >
              <Plus className="w-12 h-12 text-gray-600 group-hover:text-primary transition-colors" />
              <p className="text-gray-500 group-hover:text-primary transition-colors font-medium">
          ...
          {cards.length >= 5 && (
            ...
            >
              <p className="text-center text-sm text-gray-500">
                Hai raggiunto il limite massimo di 5 carte
              </p>
            </motion.div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card className="bg-surface/50 border-gray-800/50">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-gray-400">Carte Totali</p>
          ...
          <motion.div
            ...
            className="h-full"
          >
            <div className="h-full rounded-2xl border-2 border-dashed border-gray-700 hover:border-primary transition-all bg-surface/30 hover:bg-surface/50 flex flex-col items-center justify-center gap-3 p-6 cursor-pointer group"
              onClick={(e) => {
                e.preventDefault();
                e.stopPropagation();

                if (cards.length === 0) {
                  toast.warning('Nessuna carta salvata', {
                    message: 'Aggiungi prima una carta per selezionare quella di default!',
                    duration: 5000
                  });
                } else {
            ...
            >
              <CreditCard className="w-12 h-12 text-gray-600 group-hover:text-primary transition-colors" />
              <p className="text-gray-500 group-hover:text-primary transition-colors font-medium text-sm text-center">
          ...
          <Card className="bg-surface/50 border-gray-800/50">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-gray-400">Spese Questo Mese</p>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-xl text-white">Transazioni Recenti con Carta</CardTitle>
          </CardHeader>
          <CardContent>
            {!selectedCard ? (
      ...
      <SelectDefaultCardModal
        ...
        onSelectCard={(cardId) => {
          ...
          toast.success('Carta default impostata', {
            message: 'La carta selezionata è ora la tua carta predefinita',
            duration: 3000
          });
        }}
      />
